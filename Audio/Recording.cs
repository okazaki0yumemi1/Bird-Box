...
namespace Bird_Box.Audio
{
    public class Recording
    {
        ...
        public Recording(Microphone newInputDevice, FFMpegSettings newSettings)
        {
            ...
            {
                Console.WriteLine(
                    $"The directory {FFMpegSettings.outputPath} does not exist, creating new one."
                );
        ...
        public string RecordAudio()
        {
            ...
            if (inputDevice is not null)
            {
                var result = CommandLine.ExecuteCommand(
                    $"{settings.ffmpegExecutable} -loglevel warning -f alsa -channels 2 -sample_rate 48000 -i {inputDevice.deviceId} -t 10 {FFMpegSettings.outputPath}/{fileName}.wav"
                );
                return fileName + ".wav";
            }
            else
